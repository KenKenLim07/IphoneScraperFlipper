...
async function recheckOne(page, candidate, opts, index, total) {
  ...
  try {
    ...
    // Pass 1: compute a rough price to help the detail-extractor ignore price-only lines when inferring description.
    let derivedPriceRaw = derivePriceRawFromDetail({
      bodyText,
    ...
    if (!cleanText(derivedPriceRaw) && meta.priceAmount) {
    ...
    if (opts.logEnabled) {
      const src = primaryPriceRaw ? "above_fold" : headerPriceRaw ? "header" : meta.priceAmount && !cleanText(candidate.price_raw) ? "meta" : "text";
